<template>
    <view>
        ...
        <view class="i-m-input">
            <!--title-->
            <view class="i-title">
                <view>您收到的验证码是多少？</view>
            </view>
            <!--input verification code-->
            <u-code-input v-model="value" mode="line" :maxlength="6" :focus="true"
...
export default {
    ...
    methods: {
        ...
        submitVerification() {
            // submitVerification方法1800ms内不能重复提交
            if (this.submitVerificationLock){
                this.submitVerificationLock = false;
                setTimeout(() => {
                    this.submitVerificationLock = true;
                }, 1800);
                this.$emit("submitCode", this.value);
            }
        }
    }
};
